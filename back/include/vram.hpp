...
class Vram {
   ...
   public:
    ...
    void write(
        size_t const i,
        Word const word
    );  // don't try converting into an operator

    inline ErrType get_error(size_t const i, unsigned const bit_i) {
        return _errors[i][bit_i];
    }
    inline void
    set_error(size_t const i, unsigned const bit_i, ErrType const err) {
        _errors[i][bit_i] = err;
    }
